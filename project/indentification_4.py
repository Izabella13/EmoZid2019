...
while(1): # цикл обработки nn кадров
    ...
    if len(detections) == 0: # если в кадре никого нет выводим соответствующее сообщение
        if not ind_d:
        ...
        if j%3==0:
            if not ind_p:
                for c,fd in enumerate(fdi):
                    q = distance.euclidean(fd,face_descriptor_frame) # узнаем коэффицент похожести на определённую личность
                    if q<0.6:
...
if choice == 1: # если пользователь выбрал 1 режим мы входим в это условие
    i=0
    while i<7: # пересортировываем списки в рандомном порядке
        rand = random.randint(0,6) 
        emotions[i], emotions[rand] = emotions[rand], emotions[i] # cортируем эмоции
        pictures[i], pictures[rand] = pictures[rand], pictures[i] # cортируем изображения с подсказками
        sound[i], sound[rand] = sound[rand], sound[i] # cортируем голосовые записи из sound
        sound1[i], sound1[rand] = sound1[rand], sound1[i] # cортируем голосовые записи из sound1
        i+=1
