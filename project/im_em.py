...
def get_landmarks(image): #Функция, рассчитывающая два вида дискрипторов лица
    ...
    for k,d in enumerate(detections): #Цикл по всем найденым на изображении лицам
        ...
        landmarks_vectorised = [] #Создаем список для записи дискрипторов
        for w, z in zip(xlist, ylist): #Расчитываем дискрипторы
            ...
            landmarks_vectorised.append(dist) #Добавляем в список дискрипторы
    ...
    if len(detections) == 0: #Добавляем значения переменных, если нет выделенных лиц на экране
        ...
        face_descriptor=0
    return xlist,ylist,meannp,landmarks_vectorised,face_descriptor #Возвращаем дискрипторы, количество выделенных лиц
...
f.close()
print('Дискрипторная база создана успешно')