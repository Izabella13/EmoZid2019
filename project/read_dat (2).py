...
for i, file_name in enumerate(file_names): #Запуск цикла, пробегающегося по списку file_names, доставая и его индекс i, и то, что в нем находится
    f=open('Discript/' + file_name,'r') #открываем каждый из этих файлов с разрешением на чтение
...
clf.fit(learning_dat,l_y) #Запускаем метод обучения Support Vector Machine
